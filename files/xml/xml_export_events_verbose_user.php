...
$query = "SELECT * FROM `events` WHERE $p_lid starttime > NOW() AND created_by = $uid";
$result = mysql_query($query);
$n = mysql_num_rows($result);
$events = mysql_fetch_assoc($result);

if($n < 1) {echo"You have not created any events to export."; exit();}
...
do {
	...
	$xml->addAttribute('feed', "verbose");
	$xml->addAttribute('type', "events");
	$xml->addAttribute('time', time());
	$xml->addAttribute('n_events', $n);
	$xml->addAttribute('comments', "using this feed assumes that no other feeds are used. all tables (location, festival, stage, exhibition) are attached to each event (if relevant). not recommended for frequent updates. Please follow the terms of use for the xml exports.");
   	...
   	$event->addChild('stage_id', $events['stage_id']);
    	...
    	$event->addChild('canceled', $events['canceled']);
   	...
   	$event->addChild('ticket_url', $events['ticket_url']);
    	...
    	$event->addChild('img_original', $events['img_original']);
		...
		// prepare img-link
		if($img_original != '') { $img_link = "http://www.schmittens.net/img/location/".$img_original; }

    	
   	$location = $event->addChild('location');
   		$location->addChild('link',"http://www.schmittens.net/location.php?location_id=".$location_id); 	
   		...
   		$location->addChild('img_original', $img_original);
   	...
   	$festival = $event->addChild('festival');
		...
		if($events['f_id'] != 0) {
			...
			// prepare img-link
			if($img_original != '') { $img_link = "http://www.schmittens.net/img/festival/".$festival['img_original']; }

			$festival->addChild('link', "http://www.schmittens.net/festival.php?f_id=".$events['f_id']);
   		...
   		$festival->addChild('canceled', $festival['canceled']);
   		...
   		$festival->addChild('img_original', $festival['img_original']);
   	...
   	if($events['ex_id'] != 0) {
			...
			// prepare img-link
			if($img_original != '') { $img_link = "http://www.schmittens.net/img/exhibition/".$img_original; }
   		   		
   		$exhibition->addChild('link', "http://www.schmittens.net/exhibition.php?ex_id=".$events['ex_id']);
   		...
   		$exhibition->addChild('canceled', $canceled);
   		...
   		$exhibition->addChild('img_original', $img_original);   		
   	...
   	if($events['stage_id'] != 0) {
			...
			// prepare img-link
			if($img_original != '') { $img_link = "http://www.schmittens.net/img/stage/".$img_original; }
   		   		
   		$stage->addChild('link', "http://www.schmittens.net/stage.php?stage_id=".$events['tage_id']);
   		...
   		$stage->addChild('canceled', $canceled);   		
   		...
   		$stage->addChild('img_original', $img_original);   		
